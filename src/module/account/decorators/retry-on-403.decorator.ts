            ...
            try {
                ...
                if (isAxiosError(error) && error.response?.status === 401 && error.response?.data?.error?.code === 'UNAUTHORIZED') {
                    ...
                    if (typeof firstArg === 'string') {
                        ...
                        // Если пришел объект, считаем его сущностью
                    ...
                    try {
                        ...
                        try {
                            ...
                            // После свапа обязательно загружаем актуальное состояние из БД
                            const swapAccountWithProxy = await self.rawLoadAccountCore(accountId);

                            // Пытаемся обновить токен еще раз с новой прокси/куками
                            const refreshAfterSwap = await self.refreshPrivate(swapAccountWithProxy);

                            if (refreshAfterSwap) {
                                // Аналогично подменяем аргумент, если это был объект
                                if (typeof firstArg !== 'string') {
